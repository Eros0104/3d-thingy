...
void quat_to_mtx(float m[16], const float q[4])
{
    ...
    m[0]  = 1.0f - 2.0f*y*y - 2.0f*z*z;  m[1]  = 2.0f*x*y + 2.0f*z*w;
    m[2]  = 2.0f*x*z - 2.0f*y*w;          m[3]  = 0.0f;
    m[4]  = 2.0f*x*y - 2.0f*z*w;          m[5]  = 1.0f - 2.0f*x*x - 2.0f*z*z;
    m[6]  = 2.0f*y*z + 2.0f*x*w;          m[7]  = 0.0f;
    m[8]  = 2.0f*x*z + 2.0f*y*w;          m[9]  = 2.0f*y*z - 2.0f*x*w;
    m[10] = 1.0f - 2.0f*x*x - 2.0f*y*y;  m[11] = 0.0f;
...
void compute_local_matrix(const LocalTRS& trs, float out[16])
{
    float r[16];
    quat_to_mtx(r, trs.r);
    out[0]  = r[0]*trs.s[0];  out[1]  = r[1]*trs.s[0];  out[2]  = r[2]*trs.s[0];  out[3]  = 0.0f;
    out[4]  = r[4]*trs.s[1];  out[5]  = r[5]*trs.s[1];  out[6]  = r[6]*trs.s[1];  out[7]  = 0.0f;
    out[8]  = r[8]*trs.s[2];  out[9]  = r[9]*trs.s[2];  out[10] = r[10]*trs.s[2]; out[11] = 0.0f;
    out[12] = trs.t[0]; out[13] = trs.t[1]; out[14] = trs.t[2]; out[15] = 1.0f;
}
...
void quat_slerp(const float a[4], const float b[4], float t, float out[4])
{
    ...
    if (dot < 0.0f) { bn[0]=-b[0]; bn[1]=-b[1]; bn[2]=-b[2]; bn[3]=-b[3]; dot=-dot; }
    if (dot > 0.9995f) {
        out[0] = a[0] + t*(bn[0]-a[0]); out[1] = a[1] + t*(bn[1]-a[1]);
        out[2] = a[2] + t*(bn[2]-a[2]); out[3] = a[3] + t*(bn[3]-a[3]);
        quat_normalize(out);
        return;
    }
    ...
    out[0] = s0*a[0] + s1*bn[0]; out[1] = s0*a[1] + s1*bn[1];
    out[2] = s0*a[2] + s1*bn[2]; out[3] = s0*a[3] + s1*bn[3];
}

int find_keyframe(const std::vector<float>& input, float t, float& alpha)
{
    if (input.empty())          { alpha = 0.0f; return 0; }
    if (t <= input.front())     { alpha = 0.0f; return 0; }
    if (t >= input.back())      { alpha = 0.0f; return static_cast<int>(input.size()) - 1; }
...
RiggedModel::RiggedModel() : s_(new State())
{
    ...
    s_->layout.begin()
        ...
        // Metal rejects int4 as MTLAttributeFormatUChar4; normalized=true gives 0..1 float,
        // rescaled to 0..255 in the shader.
...
bool RiggedModel::load(const char* glb_path, std::string& err)
{
    ...
    for (size_t i = 0; i < node_count; ++i) {
        ...
        if (n.has_matrix) {
            ...
            float rm[9] = {
                sx>1e-6f ? m[0]/sx : 1.f, sx>1e-6f ? m[1]/sx : 0.f, sx>1e-6f ? m[2]/sx : 0.f,
                sy>1e-6f ? m[4]/sy : 0.f, sy>1e-6f ? m[5]/sy : 1.f, sy>1e-6f ? m[6]/sy : 0.f,
                sz>1e-6f ? m[8]/sz : 0.f, sz>1e-6f ? m[9]/sz : 0.f, sz>1e-6f ? m[10]/sz : 1.f,
            };
            float trace = rm[0] + rm[4] + rm[8];
            if (trace > 0.0f) {
                float s = std::sqrt(trace + 1.0f) * 2.0f;
                trs.r[3] = 0.25f*s; trs.r[0] = (rm[5]-rm[7])/s;
                trs.r[1] = (rm[6]-rm[2])/s; trs.r[2] = (rm[1]-rm[3])/s;
            } else if (rm[0] > rm[4] && rm[0] > rm[8]) {
                float s = std::sqrt(1.0f + rm[0] - rm[4] - rm[8]) * 2.0f;
                trs.r[3] = (rm[5]-rm[7])/s; trs.r[0] = 0.25f*s;
                trs.r[1] = (rm[3]+rm[1])/s; trs.r[2] = (rm[6]+rm[2])/s;
            } else if (rm[4] > rm[8]) {
                float s = std::sqrt(1.0f + rm[4] - rm[0] - rm[8]) * 2.0f;
                trs.r[3] = (rm[6]-rm[2])/s; trs.r[0] = (rm[3]+rm[1])/s;
                trs.r[1] = 0.25f*s;          trs.r[2] = (rm[7]+rm[5])/s;
            } else {
                float s = std::sqrt(1.0f + rm[8] - rm[0] - rm[4]) * 2.0f;
                trs.r[3] = (rm[1]-rm[3])/s; trs.r[0] = (rm[6]+rm[2])/s;
                trs.r[1] = (rm[7]+rm[5])/s; trs.r[2] = 0.25f*s;
            }
            quat_normalize(trs.r);
        } else {
            if (n.has_translation) { trs.t[0]=n.translation[0]; trs.t[1]=n.translation[1]; trs.t[2]=n.translation[2]; }
            if (n.has_rotation)    { trs.r[0]=n.rotation[0];    trs.r[1]=n.rotation[1];    trs.r[2]=n.rotation[2];    trs.r[3]=n.rotation[3]; }
            if (n.has_scale)       { trs.s[0]=n.scale[0];       trs.s[1]=n.scale[1];       trs.s[2]=n.scale[2]; }
        }
        nd.rest = trs;
        nd.current = trs;
    ...
    for (size_t i = 0; i < node_count; ++i) {
        if (s_->nodes[i].parent >= 0) continue;
    ...
    // --- Skin ---
    if (data->skins_count == 0) { cgltf_free(data); err = "glb has no skins"; return false; }
    ...
    auto get_texture = [&](const cgltf_image* img) -> bgfx::TextureHandle {
        ...
        bgfx::TextureHandle h = decode_texture_from_memory(base, img->buffer_view->size, 0);
        image_cache[idx] = h;
    ...
    for (size_t mi = 0; mi < data->meshes_count; ++mi) {
        ...
        for (size_t pi = 0; pi < mesh.primitives_count; ++pi) {
            ...
            const cgltf_accessor *accPos=nullptr, *accNrm=nullptr, *accUv=nullptr,
                                  *accJoints=nullptr, *accWeights=nullptr;
            ...
            if (!accPos || !accJoints || !accWeights) continue;
            ...
            for (size_t i = 0; i < vcount; ++i) {
                ...
                if (accNrm) { float n[3]={0,1,0}; cgltf_accessor_read_float(accNrm, i, n, 3); v.nx=n[0]; v.ny=n[1]; v.nz=n[2]; }
                if (accUv)  { float uv[2]={0,0};  cgltf_accessor_read_float(accUv,  i, uv, 2); v.u=uv[0]; v.v=uv[1]; }
                cgltf_uint j[4]={0,0,0,0}; cgltf_accessor_read_uint(accJoints, i, j, 4);
                for (int k=0; k<4; ++k) { uint32_t ji=j[k]; if (ji>=use_joints) ji=0; v.joints[k]=static_cast<uint8_t>(ji); }
                float w[4]={1,0,0,0}; cgltf_accessor_read_float(accWeights, i, w, 4);
                float ws = w[0]+w[1]+w[2]+w[3];
                if (ws > 1e-6f) { float inv=1.0f/ws; w[0]*=inv; w[1]*=inv; w[2]*=inv; w[3]*=inv; } else { w[0]=1; w[1]=w[2]=w[3]=0; }
                v.weights[0]=w[0]; v.weights[1]=w[1]; v.weights[2]=w[2]; v.weights[3]=w[3];
            }

            Primitive p;
            p.vb = bgfx::createVertexBuffer(
                bgfx::copy(verts.data(), uint32_t(vcount * sizeof(SkinnedVertex))), s_->layout);
            ...
            if (prim.material && prim.material->has_pbr_metallic_roughness) {
                ...
                for (int k=0; k<4; ++k) p.base_color[k] = pbr.base_color_factor[k];
                if (pbr.base_color_texture.texture && pbr.base_color_texture.texture->image) {
                    bgfx::TextureHandle h = get_texture(pbr.base_color_texture.texture->image);
                    if (bgfx::isValid(h)) p.texture = h;
                }
            }
    ...
    for (size_t ai = 0; ai < data->animations_count; ++ai) {
        ...
        for (size_t si = 0; si < src.samplers_count; ++si) {
            ...
            for (size_t k=0; k<ss.input->count; ++k) cgltf_accessor_read_float(ss.input, k, &d.input[k], 1);
            ...
            for (size_t k=0; k<ss.output->count; ++k)
                cgltf_accessor_read_float(ss.output, k, &d.output[k*static_cast<size_t>(d.components)], d.components);
            ...
            if (!d.input.empty()) a.duration = std::max(a.duration, d.input.back());
        }
...
void RiggedModel::play_raw(int idx, bool loop, bool restart_if_same, bool crossfade)
{
    if (!s_->loaded || idx < 0 || idx >= static_cast<int>(s_->animations.size())) return;
    if (idx == s_->current && !restart_if_same) { s_->looping = loop; return; }
    if (crossfade) s_->start_crossfade(idx);
    s_->current  = idx;
    s_->time     = 0.0f;
    s_->looping  = loop;
    s_->finished = false;
...
int         RiggedModel::anim_count()       const { return static_cast<int>(s_->animations.size()); }
const char* RiggedModel::anim_name(int i)   const {
    if (i < 0 || i >= anim_count()) return "";
...
void RiggedModel::update(float dt)
{
    ...
    if (s_->current >= 0 && s_->current < static_cast<int>(s_->animations.size())) {
        ...
        for (const AnimChannel& ch : a.channels) {
            ...
            if (smp.interp == 2) { // CUBICSPLINE: use middle (value) component, ignore tangents
            ...
            switch (ch.path) {
                ...
                case AnimPath::Rotation: {
                    float q0[4]={v0[0],v0[1],v0[2],v0[3]}, q1[4]={v1[0],v1[1],v1[2],v1[3]};
    ...
    for (size_t i = 0; i < nj; ++i)
        mat_mul(&s_->bone_matrices[i*16], s_->nodes[s_->joint_nodes[i]].world, &s_->inv_bind[i*16]);
}
    ...
    uint64_t state, const ViewmodelDrawParams& p)
{
    ...
    mat_mul(rxz, ry, rx); mat_mul(rxyz, rxz, rz); mat_mul(rotTweak, rxyz, scl);

    float trnOffset[16]; bx::mtxIdentity(trnOffset);
    trnOffset[12]=p.offset[0]; trnOffset[13]=p.offset[1]; trnOffset[14]=p.offset[2];
    float local[16]; mat_mul(local, trnOffset, rotTweak);
    ...
    float model[16]; mat_mul(model, camRot, local);
    model[12] += p.eye[0]; model[13] += p.eye[1]; model[14] += p.eye[2];
    ...
    for (const Primitive& prim : s_->primitives) {
        ...
        bgfx::setTexture(0, s_albedo, bgfx::isValid(prim.texture) ? prim.texture : fallback_white);
...
void RiggedModel::submit_axes_gizmo(
    bgfx::ViewId view_id, bgfx::ProgramHandle debug_program,
    const ViewmodelDrawParams& p, float length)
{
    if (!bgfx::isValid(debug_program)) return;

    float rx[16], ry[16], rz[16], rxz[16], rxyz[16];
    bx::mtxRotateX(rx, p.tweak_pitch); bx::mtxRotateY(ry, p.tweak_yaw); bx::mtxRotateZ(rz, p.tweak_roll);
    mat_mul(rxz, ry, rx); mat_mul(rxyz, rxz, rz);

    float trnOffset[16]; bx::mtxIdentity(trnOffset);
    trnOffset[12]=p.offset[0]; trnOffset[13]=p.offset[1]; trnOffset[14]=p.offset[2];
    float local[16]; mat_mul(local, trnOffset, rxyz);

    float camPitch[16], camYaw[16], camRot[16];
    bx::mtxRotateX(camPitch, -p.pitch); bx::mtxRotateY(camYaw, p.yaw);
    mat_mul(camRot, camYaw, camPitch);
    ...
    v[0]={0,0,0,0xff0000ffu}; v[1]={length,0,0,0xff0000ffu};
    v[2]={0,0,0,0xff00ff00u}; v[3]={0,length,0,0xff00ff00u};
    v[4]={0,0,0,0xffff0000u}; v[5]={0,0,length,0xffff0000u};
