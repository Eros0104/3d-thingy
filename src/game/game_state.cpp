...
bool GameState::init(const char* level_path, int width, int height) {
    ...
    // Audio
    {
        ...
        if (!engine::audio_init(err)) {
            ...
            shot_sound_ = engine::audio_load(ENGINE_SOUNDS_DIR "/pistol_shot.mp3", err);
    ...
    // Uniforms
    u_light_pos_    = bgfx::createUniform("u_lightPos",   bgfx::UniformType::Vec4, k_max_shader_lights);
    u_light_color_  = bgfx::createUniform("u_lightColor", bgfx::UniformType::Vec4, k_max_shader_lights);
    u_light_params_ = bgfx::createUniform("u_lightParams",bgfx::UniformType::Vec4);
    ...
    wall_tex_ = engine::load_texture_from_file(
    ...
    // Light bulb sphere
    {
        ...
        bulb_vbh_ = bgfx::createVertexBuffer(
            bgfx::copy(verts.data(),   uint32_t(verts.size()   * sizeof(LitVertex))), layout_);
        bulb_ibh_ = bgfx::createIndexBuffer(
    ...
    // Target cube VBHs (one per damage tint, shared IB)
    {
        constexpr uint32_t tints[k_target_max_hits] = {
            0xff5050ffu, 0xff3030c0u, 0xff1a1a80u,
        };
        std::vector<LitVertex> verts;
        std::vector<uint16_t>  indices;
    ...
    camera_.eyeY = level_.spawn.pos.y + engine::PlayerPhysics::k_eye_height + 0.02f;
...
void GameState::shutdown() {
    ...
    for (auto& m : models_) m->unload();
    models_.clear();

    auto dvb = [](bgfx::VertexBufferHandle h) { if (bgfx::isValid(h)) bgfx::destroy(h); };
    auto dib = [](bgfx::IndexBufferHandle  h) { if (bgfx::isValid(h)) bgfx::destroy(h); };
    auto dph = [](bgfx::ProgramHandle      h) { if (bgfx::isValid(h)) bgfx::destroy(h); };
    auto dth = [](bgfx::TextureHandle      h) { if (bgfx::isValid(h)) bgfx::destroy(h); };
    auto duh = [](bgfx::UniformHandle      h) { if (bgfx::isValid(h)) bgfx::destroy(h); };

    dvb(floor_vbh_); dvb(wall_vbh_); dvb(stair_vbh_);
    dvb(bulb_vbh_);  dib(bulb_ibh_);
    for (int t = 0; t < k_target_max_hits; ++t) dvb(cube_vbh_[t]);
    dib(cube_ibh_);

    dth(floor_tex_); dth(wall_tex_); dth(white_tex_);

    duh(u_light_pos_); duh(u_light_color_); duh(u_light_params_);
    duh(u_ambient_);   duh(s_albedo_);       duh(u_bones_);
    duh(u_baseColor_);

    dph(program_); dph(skinned_program_); dph(debug_program_); dph(hud_program_);
}
...
entt::entity GameState::spawn_zombie(float x, float y, float z) {
    ...
    tr.x = x; tr.y = y; tr.z = z; tr.yaw = 0.f;
...
void GameState::update(float dt) {
    ...
    engine::player_physics_step(camera_, player_physics_, dt, level_, prev_x, prev_z);
...
void GameState::sys_shooting() {
    if (!shoot_pressed_) return;
    const bool can_shoot = !is_reloading_ && bullets_in_clip_ > 0;
    if (!can_shoot) { shoot_pressed_ = false; return; }
    ...
    engine::ray_walls_nearest(level_.walls,
        camera_.eyeX, camera_.eyeY, camera_.eyeZ, fx, fy, fz, best_t);
    ...
    // Targets: find closest, damage it.
    {
        ...
        for (auto entity : registry_.view<game::TargetTag>()) {
            ...
            if (engine::ray_aabb(camera_.eyeX, camera_.eyeY, camera_.eyeZ, fx, fy, fz,
                                 tr.x-h, tr.y-h, tr.z-h,
                                 tr.x+h, tr.y+h, tr.z+h, t_hit) && t_hit < best_t) {
                best_t = t_hit;
                best_e = entity;
            }
        }
...
void GameState::sys_zombie_ai(float dt) {
    ...
    for (auto entity : registry_.view<game::ZombieTag>()) {
        ...
        if (ai.hit_stun_timer > 0.f) {
            ...
            if (ai.hit_stun_timer < 0.f) ai.hit_stun_timer = 0.f;
...
void GameState::render(int width, int height) {
    ...
    fps_camera_view_proj(camera_, aspect, bgfx::getCaps()->homogeneousDepth, view, proj);
...
void GameState::sys_set_lights() {
    ...
    if (n == 0) {
        pos_pack[0]=0.f; pos_pack[1]=4.f; pos_pack[2]=0.f; pos_pack[3]=0.f;
        col_pack[0]=2.4f; col_pack[1]=2.1f; col_pack[2]=1.7f; col_pack[3]=0.f;
        n = 1;
    } else {
    ...
    const float amb[4]    = {level_.ambient[0], level_.ambient[1], level_.ambient[2], 0.f};
...
void GameState::sys_render_level() {
    ...
    const bgfx::TextureHandle floor_bind = bgfx::isValid(floor_tex_) ? floor_tex_ : white_tex_;
    const bgfx::TextureHandle wall_bind  = bgfx::isValid(wall_tex_)  ? wall_tex_  : white_tex_;
    ...
    if (level_.lights.empty()) {
        ...
        for (const auto& L : level_.lights) draw_bulb(L.pos.x, L.pos.y, L.pos.z);
    }
}
...
void GameState::sys_render_targets() {
    ...
    for (auto entity : registry_.view<game::TargetTag>()) {
        ...
        if (!tgt.alive) continue;
        const int tint = std::clamp(k_target_max_hits - tgt.hits_remaining, 0, k_target_max_hits - 1);
        bgfx::setState(render_state_);
...
void GameState::sys_render_hud() {
    ...
    const float baseline_y    = float(height_) - margin_bottom + engine::hud_descent();
    ...
    engine::hud_draw_text_right(buf, float(width_) - margin_x, baseline_y, k_hud_red);

    constexpr float dot = 3.f;
    engine::hud_draw_solid_rect(
        (float(width_) - dot) * 0.5f,
        (float(height_) - dot) * 0.5f,
        dot, dot, 0xffffffffu);
}
...
void GameState::sys_render_debug_text() {
    ...
    bgfx::dbgTextPrintf(0, 1, 0x0f,
        "WASD  Mouse  Esc: %s   G: gizmo=%s   LMB:Shoot R:Reload",
        mouse_look_ ? "free cursor" : "capture",
        show_axes_gizmo_ ? "on" : "off");
    bgfx::dbgTextPrintf(0, 2, 0x0f,
        "level=%s  sectors=%zu walls=%zu stairs=%zu",
        level_.name.empty() ? "(unnamed)" : level_.name.c_str(),
        level_.sectors.size(), level_.walls.size(), level_.stairs.size());
}
