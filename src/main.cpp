...
int main(int argc, char **argv) {
  ...
  if (!application::init("fps-engine (SDL2 + bgfx)", 1280, 720)) {
    return 1;
  }
  ...
  int width = application::get_current_window_width();
  int height = application::get_current_window_height();
  ...
  while (application::is_running()) {
    ...
    while (SDL_PollEvent(&event)) {
      if (event.type == SDL_QUIT) {
        ...
        width = application::get_current_window_width();
        height = application::get_current_window_height();
      }
    }
    ...
    if (shoot_pressed && can_shoot) {
      ...
      engine::ray_walls_nearest(level, camera.eyeX, camera.eyeY, camera.eyeZ,
                                fx, fy, fz, wall_t);
